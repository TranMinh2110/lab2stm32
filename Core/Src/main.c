...
//  BSP_LCD_DisplayStringAtLine(4,"LCD DEMO");
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  BSP_GYRO_GetXYZ(val);
	  sprintf(LCD_send, "x = %f", val[0]);
	  BSP_LCD_DisplayStringAtLine(2, LCD_send);
	  sprintf(LCD_send, "y = %f", val[1]);
	  BSP_LCD_DisplayStringAtLine(3, LCD_send);
	  sprintf(LCD_send, "z = %f", val[2]);
	  BSP_LCD_DisplayStringAtLine(4, LCD_send);
	  HAL_Delay(1000);
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
